...
| 权限归属台账 | `crew`（`/crew-ledger` 页面） | 岗位权限与进出流水 | 岗位职务、可办理动作、进出记录 |
...
剧组成员的进场、离场、请假按**经办人岗位职务**授权（经办人通过请求头
`X-Operator-Id` 或请求体 `operatorId` 指定，值为剧组成员 id）：

| 岗位 | 办理进场 | 办理离场 | 登记请假 | 归属变更 |
| --- | --- | --- | --- | --- |
| 制片（岗位含“制片”） | ✅ | ✅ | ✅ | ✅ |
| 场务（岗位含“场务”） | ✅ | ❌ | ❌ | ❌ |
| 演员统筹 | ❌ | ❌ | ✅ | ❌ |
...
- 越权请求返回 `403`；未指定经办人、只读成员的写操作同样被拒绝。
- 状态冲突（如待进场请假、**重复离场**、重复请假）返回 `409`；同一成员的
  动作在 per-member 锁内“先校验后落库”，多人并发办理离场只有一个成功。
- `GET /api/crew/ledger` 返回权限矩阵、在组统计、成员权限归属与进出流水；
  人员列表、`/api/crew/stats` 统计与台账读同一份 `crew` 数据，
  归属（岗位/组别）调整后各处立即一致。
- 每次成功动作向 `crew_history` 追加一条流水，只追加不改写，历史进出记录保持原样。
- 后端验收测试：`cd backend && .venv/bin/python -m pytest tests/`。
